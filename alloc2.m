function [out] = alloc2(I,T,N,P,Fe,Pmax,...
...
% Qn_max = 0.28.*Qc.*ones(size(N));
% Nstor_max2 = max(0,Qn_max-Qn_nsto);
...
% indx_maxNstor = find(Nstor > Nstor_max2); 
% Nstor(indx_maxNstor) = Nstor_max2(indx_maxNstor); % reset to maximum value
% Nexc= zeros(size(P)); 
% idx_n = Nstor > Nstor_max2;
% Nexc(idx_n) = Nstor(idx_n)-Nstor_max2();
Nstor_max = 0.4e-15;
Qn_max = Qn_nsto+Nstor_max;
Nstor = zeros(size(N)); % initialize vector of zeros
indx = find(limType~=1& limType~=4); % find not N limited locations
...
Pstor_max = max(0,Qp_max1-Qp_nsto);
